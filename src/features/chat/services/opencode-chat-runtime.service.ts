...
export const chatContextSystemPrompt = [
  ...
  "Use create_action_draft when the operator asks you to prepare or propose an nmap, nuclei, or ffuf scanner action for later inspection. Before creating a draft, use get_session_context and put the real target in command/form state instead of placeholders such as <TARGET>.",
  "When inspect_page is available, use it only for a relevant exact-origin public page unless the operator has explicitly selected one accepted authentication context for the next inspection. Use authenticationMode accepted_context only for that one selection. It is read-only, does not persist results, and may return partial or truncated sections.",
