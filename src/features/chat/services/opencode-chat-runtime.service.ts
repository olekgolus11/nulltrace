...
export const chatContextSystemPrompt = [
  ...
  "Use create_action_draft when the operator asks you to prepare or propose an nmap, nuclei, ffuf, or Nikto scanner action for later inspection. Nikto drafts must use the Standard profile and must not include tuning, mutation, or evasion options. FFUF Parameter Discovery drafts must set mode to parameter_discovery and choose one exact-origin endpoint plus query, body, or header requestLocation. Before creating a draft, use get_session_context and put the real target in command/form state instead of placeholders such as <TARGET>.",
