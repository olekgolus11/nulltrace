...
Use `linux/amd64` only after building that architecture. The qualification starts two controlled host receivers. It proves that the approved request arrives, a redirect to the forbidden receiver produces zero receiver requests, a `--noproxy` direct attempt produces zero receiver requests, and direct DNS plus unauthorized IPv6 fail. It also requires cleanup confirmation. The generated local evidence is ignored; sanitized reviewed evidence is stored under `infrastructure/isolation/evidence/`.

OrbStack 2026-09-21 on Linux ARM64 is qualified in this change. Docker Desktop, Linux AMD64, public IPv6, LAN/VPN routes and enforcing AppArmor/SELinux profiles remain unqualified. The engine default seccomp profile was active through Docker's default behavior; this change does not ship a custom seccomp profile. No TLS interception is used.

## Remaining stages

The runtime still needs broker-daemon supervision and restart reconciliation, cancellation-specific tests, bounded artifact transfer and per-tool adapters. cURL is the first vertical slice. Playwright must additionally cover redirects, subresources, `fetch`, frames, popups, WebSockets, service workers and downloads. Nmap requires its own non-proxy profile. Nuclei credentials must retain ephemeral secret-file delivery and exact-origin semantics.
