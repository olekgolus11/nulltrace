...
Parsing runs inside the bounded worker/result boundary; enforce HTML/XML, queue/frontier, URL and aggregate result limits before large allocations. Return incremental validated entries and a non-secret checkpoint. The application owns commits/deduplication. A worker cannot upsert another target or request arbitrary URLs through forged checkpoint state. Query/path data can contain credentials: do not label raw URLs non-secret by default. Use validated canonical checkpoint metadata and run-scoped opaque references for secret-bearing addresses. If a reference cannot be safely recovered after restart, report that part of the frontier unavailable and rediscover it through an authorized crawl; never persist plaintext secrets or request a redacted URL as if it were the original. HTTP/body timeouts cover body consumption, including chunked/stalled data after headers.
